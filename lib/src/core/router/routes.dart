class Routes {
  // Splash View
  static const String splashScreen = '/';

  // Onboarding View
  static const String onboardingScreen = '/onboarding-screen';

  // Add User Screen
  static const String searchScreen = '/search-list-screen';

  // Otp Screen
  static const String optScreen = '/opt-screen';
  // Dashboard View
  static const String dashboardScreen = '/Dashboard-screen';

// Notifications View
  static const String notificationsScreen = '/notifications-screen';

  // Settings View
  static const String settingsScreen = '/settings-screen';

  // Intro View
  static const String introScreen = '/intro-screen';

  // Login View
  static const String signinScreen = '/login-screen';

  // Signup View
  static const String signupScreen = '/signup-screen';

  // Confirm Mail View
  static const String confirmMailScreen = '/confirm-mail-screen';

  // Reset Password View
  static const String resetPasswordScreen = '/reset-password-screen';

  // Home View
  static const String homeScreen = '/home-screen';

  /// Drawer Router
  static const String analyticScreen = '/analytics-screen';

  /// Drawer Router
  static const String addPropertyScreen = '/add-property-screen';

  /// Property List Screen
  static const String propertyListScreen = '/property-list-screen';

  //Properties detail page
  static const String propertyDetailScreen = '/property-detail-view';

  // Lead  view page
  static const String leadScreen = '/lead-view';

  // Create Lead Form page
  static const String createLeadFormScreen = '/create-lead-form-screen';

  //Followup view page
  static const String followupScreen = '/followup-view';

  // Create follow form page
  static const String followupFormScreen = '/create-followup-form-screen';

  // Show notification  page
  static const String notificationScreen = '/show-notification-screen';
  // tearm and condition view page
  static const String termsAndConditionScreen = '/terms-and-condition-screen';

  // FAQ view page
  static const String faqScreen = '/faq-screen';

  //feedback view page
  static const String feedbackScreen = '/feedback-screen';

  // Help and support view page
  static const String helpAndSupportScreen = '/help-and-support-screen';
  // report bug screen
  static const String reportBugScreen = '/report-bug-screen';

  // About us screen
  static const String aboutUsScreen = '/about-us-screen';
}
